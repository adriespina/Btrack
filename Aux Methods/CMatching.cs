...
namespace Billetrack
{
    ...
    public class TaskInfo2
    {
        ...
        public string _candidate;
        public int _modo;
        public TaskInfo2(CSurf model, string candidate, int modo)
        {
            _model = model;
            _candidate = candidate;
            _modo = modo;
   ...
   public  class CMatching
    {
        ...
        public int MATCHING_TIMEOUT = 1000000;
    ...
    public CMatching( int threads)
    {
     ...
     public int MatchingOneToVarius2(string filename, ref string[] filenameMatrixObjects, out resultMatching[] pResult)
     {
         ...
         try
         {
             ...
             foreach (string pos_imagen in filenameMatrixObjects)
             {
                 tareas[index] = smartThreadPool.QueueWorkItem(new WorkItemCallback(this.Match2), new TaskInfo2(modelos[index], pos_imagen, CSurf.MODE_CESAR));
                 index++;
             ...
             //Si no pruebo con el segundo metodo
                 else
                 {
                     ...
                     foreach (string pos_imagen in filenameMatrixObjects)
                     {
                         tareas[index] = smartThreadPool.QueueWorkItem(new WorkItemCallback(this.Match2), new TaskInfo2(modelos[index], pos_imagen, CSurf.MODE_SURF));
                         index++;
        ...
        resultMatching Match(Object State)
        {
            try
            {
                ...
                try
                {
                     model = new CSurf(filename, -1, -1);
                }
                catch (Exception e)
                {
                    model = null;
                    if (this._padre != null) this._padre.AddLogError("Error creando descriptores archivo original : "+filename+"Error : "+e.Message);
                }
        ...
        resultMatching Match2(Object State)
        {
            try
            {
